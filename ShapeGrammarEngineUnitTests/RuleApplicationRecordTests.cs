...
namespace ShapeGrammarEngine.UnitTests
{
	class RuleApplicationRecordTests
	{
		...
		{
			Assert.Throws<ArgumentNullException>(() => new RuleApplicationRecord(null, PolylineGeometry.CreateEmptyPolylineGeometry(), new LabelingDictionary()));
			Assert.Throws<ArgumentNullException>(() => new RuleApplicationRecord(PolylineGeometry.CreateEmptyPolylineGeometry(), null, new LabelingDictionary()));
